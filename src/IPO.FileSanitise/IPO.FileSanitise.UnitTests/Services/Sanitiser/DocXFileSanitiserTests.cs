...
namespace IPO.FileSanitise.UnitTests.Services.Sanitiser
{
    ...
    public class DocXFileSanitiserTests
    {
        #region Test Setup

        private readonly DocXFileSanitiser _uut;
        private const string DocName = "test.docx";

        public DocXFileSanitiserTests()
        {
            _uut = new DocXFileSanitiser();
        }
        ...
        {
            // Arrange
            var file = DocumentBuilder.CreateDocument(DocName, Resources.TestFileDocX);
        ...
        public void Sanitiser_ReturnsCorrectResultModelForValidFile()
        {
            ...
            result.Updated.ContentType.Should().Be("application/vnd.openxmlformats-officedocument.wordprocessingml.document");
        }
    }
}