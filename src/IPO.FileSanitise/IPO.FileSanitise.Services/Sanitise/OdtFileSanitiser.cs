...
{
    public class OdtFileSanitiser : IFileSanitiser
    {
        public virtual FileSanitiseResult Sanitise(IFormFile file)
        {
            using (var fileStream = file.OpenReadStream())
            {
                if (!fileStream.TryLoadWordFile(LoadFormat.Odt, out var document) || document == null)
                {
                    return FileSanitiseResult.CreateFailedToLoadResult();
                }

                var updatedFile = document.ClearAllAndUpdate(file, SaveFormat.Odt);
